    ...
    plot.hooks.draw.push( function (plot, ctx) {
	  ...
	  if(val == null){val=''}
          ...
          if (!hideSame || val != last_val || i == series.data.length - 1) {
            ...
            /**********Multi bar series, start********************
              基本思路：根据 series 的序号，重新计算 xx 的数值。
              
              一些限制：
              1, bars.barWidth 要设置为相等
              2, bars.align = 'left', 或者不设置，默认值即可
              3, valueLabels.align = 'center'
              4, 只能有一个xAxis
              5, 如果只有一个bar series, 那么要设置bars.align = 'center'
            ...
            **********************************/
            var myXAxis = plot.getXAxes(1)[0];
            var seriesCount = plot.getData().length;
            var barWidthPixel = myXAxis.box.width / (myXAxis.max - myXAxis.min) * series.bars.barWidth;

            if(seriesCount > 1 && seriesCount != (ii * 2 + 1) ) {

              if(seriesCount % 2 == 0) {

                xx = xx - barWidthPixel * (seriesCount / 2 - 0.5 - ii);

              }else {

                if(ii != seriesCount / 2) {
            
                  xx = xx - barWidthPixel * ((seriesCount - 1) / 2 - ii);
            
                }
              }
            }
            /**********Multi bar series, end********************/
